...
Get-ChildItem $PublishPath | Remove-Item -Force -Recurse -ErrorAction SilentlyContinue

dotnet publish .\SCD.sln -c Release -p:PublishProfile=win-x64 -v q --nologo
Compress-Archive -Path $PublishPath\win-x64 -DestinationPath $PublishPath\win-x64
...
Compress-Archive -Path $PublishPath\win-x86 -DestinationPath $PublishPath\win-x86
...
Compress-Archive -Path $PublishPath\win-arm64 -DestinationPath $PublishPath\win-arm64
...
Compress-Archive -Path $PublishPath\win-arm -DestinationPath $PublishPath\win-arm
...
Compress-Archive -Path $PublishPath\linux-x64 -DestinationPath $PublishPath\linux-x64
...
Compress-Archive -Path $PublishPath\osx-x64 -DestinationPath $PublishPath\osx-x64
